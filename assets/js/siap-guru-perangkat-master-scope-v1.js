...
(()=>{
  ...
  const activeRow=(room)=>{
    const value=text(room.querySelector('#sgPaTopic')?.value),rows=scopeRows(room);
    return rows.find(r=>text(r.bab)===value)||null;
  };
  ...
  const collectRecords=()=>{
    ...
    const walk=(x,d=0,parent={},parentTopic='',parentSemester='')=>{
      ...
      const meta=add(x,parent,parentTopic,parentSemester);
      const next={mapel:meta.mapel||parent.mapel||'',kelas:meta.kelas||parent.kelas||''};
  ...
  const activeRecords=(room,row)=>{
    const all=collectRecords(),s=selected();
    const cls=kelas(room.querySelector('#sgPaClass')?.value)||kelas(s?.kelas),mapel=norm(room.querySelector('#sgPaSubject')?.value)||norm(s?.mapel||s?.mapelName),rows=scopeRows(room),idx=rowIndex(row,rows),title=norm(row?.bab);
    const scoped=all.filter(r=>(!cls||!r.kelas||kelas(r.kelas)===cls)&&(!mapel||!r.mapel||norm(r.mapel)===mapel));
    const byTopic=scoped.filter(r=>{
      const t=norm(r.topic);
      if(!t)return false;
      if(t===title)return true;
      if(idx&&new RegExp(`^bab\\s*${idx}(?:\\b|\\s*[-—:])`,'i').test(t))return true;
      return title&&t.includes(title);
    });
    return byTopic;
  };
  const manualContext=(topic)=>{
    const db=read('siapguru_perangkat_bab_context_v1')||{},v=db[text(topic)]||{};
    const bad=/SEMUA BAB \/ 1 TAHUN|peserta didik mampu/i;
    return v.manual===true&&!Object.values(v).some(x=>bad.test(text(x)))?v:{};
  };
  const repairContext=()=>{
    const room=document.querySelector(ROOM),row=room&&activeRow(room);
    if(!room||!row)return;
    const records=activeRecords(room,row),saved=manualContext(row.bab),tp=records.filter(r=>r.tp);
    const masterJp=Number(row.jp||0)||8,totalTpJp=tp.reduce((n,r)=>n+(Number(r.jp)||0),0);
    const jp=saved.jp?Number(saved.jp):totalTpJp||masterJp;
    ...
    const topic=text(row.bab);
    const setIfNoManual=(id,value)=>{if(!saved[id])set(id,value)};
  ...
  const rebuild=()=>{
    ...
    if(sel.dataset.sgMasterScope!==signature){sel.innerHTML=html;sel.dataset.sgMasterScope=signature;if(current&&rows.some(r=>text(r.bab)===current))sel.value=current;else if(rows.length)sel.value=rows[0].bab}
    repairContext();
  };
  let timer=0;
  const run=()=>{clearTimeout(timer);timer=setTimeout(rebuild,80)};
  const boot=()=>{
    run();
    if(!window.__sgPerangkatMasterScopeWatch){
      window.__sgPerangkatMasterScopeWatch=1;
      new MutationObserver(run).observe(document.body,{childList:true,subtree:true});
      document.addEventListener('change',e=>{if(e.target?.id==='sgPaClass'||e.target?.id==='sgPaSubject'||e.target?.id==='sgPaTopic')run()},true);
      document.addEventListener('input',e=>{if(/^sgPa(JP|Media|Source|Notes|Lkpd|Assessment|Differentiation|Followup)$/.test(e.target?.id||''))run()},true);
    }
  };
