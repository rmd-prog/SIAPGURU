(()=>{
  ...
  const boot=()=>{
    ...
    const genericMaterial=/^(materi pembelajaran|materi|pembelajaran)$/i;
    const extractBab=text=>{
      const s=clean(text);
      let m=s.match(/\b(BAB\s*\d+\s*[—–-]\s*[^:;.!?]+?)(?=\s*:|\s*—\s*Peserta|\s*Peserta|$)/i);
      if(m)return clean(m[1]);
      m=s.match(/\b(BAB\s*\d+\s*[:.]\s*[^;.!?]+)/i);
      if(m)return clean(m[1]);
      m=s.match(/\b(BAB\s*\d+)\b/i);
      return m?clean(m[1]):'';
    };
    const material=x=>{
      const direct=[x.topic,x.materi,x.material,x.submateri,x.bab].map(clean).find(v=>v&&!genericMaterial.test(v));
      return direct||extractBab(x.text)||'Materi pembelajaran';
    };
    const assessment=x=>{
      const a=clean(x.assessment||x.asesmen);
      if(a&&!/^asesmen formatif$/i.test(a)&&!/^formatif$/i.test(a)&&!/^asesmen sumatif$/i.test(a)&&!/^sumatif$/i.test(a))return a;
      const s=(clean(x.text)+' '+clean(x.materi)+' '+clean(x.topic)).toLowerCase();
      if(/sumatif|akhir semester|ujian|ulangan/.test(s))return 'Asesmen sumatif';
      if(/proyek|project|produk/.test(s))return 'Asesmen proyek / produk';
      if(/praktik|praktek|unjuk kerja/.test(s))return 'Asesmen praktik / unjuk kerja';
      if(/presentasi|menyajikan/.test(s))return 'Asesmen presentasi';
      return 'Asesmen formatif';
    };
    const note=x=>{
      const raw=clean(x.note||x.keterangan);
      if(raw&&!/^(pembelajaran reguler|—|-)$/i.test(raw))return raw;
      const s=(clean(x.text)+' '+clean(x.materi)+' '+clean(x.topic)+' '+clean(x.assessment||x.asesmen)).toLowerCase();
      if(/sumatif|akhir semester|ujian|ulangan/.test(s))return 'Evaluasi akhir semester';
      if(/remedial|perbaikan/.test(s))return 'Remedial';
      if(/pengayaan|enrichment/.test(s))return 'Pengayaan';
      if(/proyek|project|produk/.test(s))return 'Proyek / unjuk karya';
      if(/praktik|praktek|unjuk kerja/.test(s))return 'Praktik / unjuk kerja';
      if(/menyajikan|presentasi/.test(s))return 'Presentasi / refleksi';
      if(/menerapkan|menggunakan|melakukan/.test(s))return 'Latihan dan penerapan';
      if(/menganalisis|menjelaskan|membandingkan/.test(s))return 'Penguatan dan refleksi';
      if(/memahami|mengenal|mengidentifikasi/.test(s))return 'Pemahaman konsep';
      return 'Pembelajaran dan asesmen formatif';
    };
    const normalize=x=>({...x,materi:material(x),assessment:assessment(x),note:note(x)});
    const markSemesterEnds=items=>{[1,2].forEach(sem=>{const rows=items.filter(x=>String(x.semester)===String(sem));if(!rows.length)return;const last=rows[rows.length-1];if(!clean(last._explicitAssessment))last.assessment='Asesmen sumatif akhir semester';last.note='Evaluasi akhir semester';});return items};
    const renderRows=sem=>{const list=$('sgProsemList'+sem),rows=state.items.map((x,i)=>({x,i})).filter(o=>String(o.x.semester)===String(sem));$('sgProsemCount'+sem).textContent=rows.length+' rencana';if(!rows.length){list.innerHTML='<tr><td colspan="8" class="sg-prosem-empty">Belum ada data Semester '+sem+'.</td></tr>';return;}list.innerHTML=rows.map((o,n)=>{const x=o.x,i=o.i;return `<tr><td>${n+1}</td><td><input data-field="period" data-i="${i}" value="${esc(x.period)}" placeholder="Juli M3"></td><td><input data-field="materi" data-i="${i}" value="${esc(x.materi||material(x))}" placeholder="BAB / materi pokok"></td><td><textarea data-field="text" data-i="${i}">${esc(x.text)}</textarea></td><td><input class="sg-prosem-jp" type="number" min="1" data-field="jp" data-i="${i}" value="${Number(x.jp)||1}"></td><td><input data-field="assessment" data-i="${i}" value="${esc(x.assessment||assessment(x))}" placeholder="Asesmen formatif / sumatif"></td><td><input data-field="note" data-i="${i}" value="${esc(x.note||'')}" placeholder="Otomatis berdasarkan TP / asesmen"></td><td><button type="button" class="sg-prosem-del" data-del="${i}">Hapus</button></td></tr>`}).join('');};
    ...
    const fromProta=()=>{const p=readProta();if(!p||!Array.isArray(p.items)||!p.items.length)return false;state.school=p.school||state.school;state.subject=p.subject||'';state.phase=p.phase||'';state.class=p.class||'';state.year=p.year||'2026/2027';state.weeks=Number(p.weeks)||36;state.items=p.items.map(x=>normalize({text:x.text||'',semester:String(x.semester||'1'),period:x.period||'',jp:Math.max(1,Number(x.jp)||2),topic:x.topic||'',materi:x.materi||'',assessment:x.assessment||x.asesmen||'',_explicitAssessment:clean(x.assessment||x.asesmen)}));markSemesterEnds(state.items);$('sgProsemSchool').value=state.school;$('sgProsemSubject').value=state.subject||'-';$('sgProsemPhase').value=state.phase||'-';$('sgProsemClass').value=state.class||'-';$('sgProsemYear').value=state.year;$('sgProsemWeeks').value=state.weeks;$('sgProsemNotes').value=state.notes;render();return true;};
    $('sgProsemLoad').onclick=()=>{if(fromProta())show(state.items.length+' rencana tersusun dari PROTA aktif. BAB, asesmen, dan keterangan dipetakan ulang dari TP.');else show('Belum ada PROTA tersimpan. Susun PROTA terlebih dahulu.');};
    $('sgProsemAdd').onclick=()=>{sync();state.items.push({text:'',semester:state.items.some(x=>String(x.semester)==='1')?'2':'1',period:'',jp:2,topic:'',materi:'',assessment:'Asesmen formatif',note:'Pembelajaran dan asesmen formatif'});render();};
    ...
    $('sgProsemSave').onclick=()=>{sync();if(!state.items.length){show('Tambahkan minimal satu rencana.');return;}localStorage.setItem(KEY,JSON.stringify({...state,savedAt:new Date().toISOString(),version:'PROSEM-9'}));show('PROSEM tersimpan.');};
