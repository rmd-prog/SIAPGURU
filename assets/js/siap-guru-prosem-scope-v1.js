...
(()=>{
  ...
  const apply=room=>{
    ...
    const cards=[...room.querySelectorAll('.sg-v4-card')];
    ...
    (room.querySelector('.sg-v4-actions')||cards[0]).appendChild(box);
    ...
    const refresh=()=>{
      ...
      cards[0].hidden=v==='2';
      cards[1].hidden=v==='1';
      if(v==='year'){cards[0].hidden=false;cards[1].hidden=false}
      save(v);
    };
    select.addEventListener('change',refresh);
    refresh();
  };
  const scan=()=>document.querySelectorAll('.sg-prosem-matrix-room').forEach(apply);
  scan();
