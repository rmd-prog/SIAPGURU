(()=>{
/* SIAP GURU — TP Subtopic Bridge v1
   Menurunkan BAB -> submateri -> TP yang berbeda.
   Non-invasive: hanya menyusun draft TP dari Master BAB. */
...
const build=rows=>{const out=[];rows.forEach((r,bi)=>{const ss=subtopics(r),sem=String(r.semester)==='2'?'2':'1',base=Math.max(1,Number(r.jp)||8),jp=Math.max(1,Math.floor(base/ss.length));ss.forEach((materi,i)=>{const verb=i===0?'mengenali dan menjelaskan':i===1?'mengidentifikasi dan memahami':i===2?'menggunakan':i===3?'menerapkan':i===4?'mengomunikasikan':'merefleksikan dan memperbaiki pemahaman';out.push({id:`chain-sub-${r.id||bi}-${i}`,text:`Peserta didik mampu ${verb} ${materi.toLowerCase()} melalui kegiatan belajar yang kontekstual dan bermakna.`,element:'',cp:'',jp,semester:sem,topic:String(r.bab||`Bab ${bi+1}`),materi,subtopik:materi,acdId:r.id||'',period:''})})});return out};
