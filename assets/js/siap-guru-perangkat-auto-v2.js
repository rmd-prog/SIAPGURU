...
const apply=(room,topic)=>{
 ...
 const source=manual&&saved.source?saved.source:(first.source||`Sumber belajar ${topic}: Buku teks/SIBI, lingkungan sekitar, dan sumber digital yang relevan.`);
 const notes=manual&&saved.notes?saved.notes:'';
 const lkpd=manual&&saved.lkpd?saved.lkpd:`LKPD ${topic}: kegiatan memahami konsep, latihan terarah, dan penerapan sesuai TP.`;
 const assessment=manual&&saved.assessment?saved.assessment:`Asesmen ${topic}: cek pemahaman, tugas/unjuk kerja, dan asesmen sesuai TP.`;
 const diff=manual&&saved.differentiation?saved.differentiation:`Diferensiasi ${topic}: dukungan bertahap bagi peserta didik yang memerlukan bantuan dan pengayaan bagi yang sudah mencapai tujuan.`;
 const follow=manual&&saved.followup?saved.followup:`Remedial dan pengayaan ${topic} berdasarkan hasil asesmen dan ketercapaian TP.`;
 const set=(id,v)=>{const e=room.querySelector('#'+id);if(e)e.value=v};
 applying=true;
 set('sgPaTopic',topic);set('sgPaJP',jp);set('sgPaMaterial',material);set('sgPaMedia',media);set('sgPaSource',source);set('sgPaNotes',notes);set('sgPaLkpd',lkpd);set('sgPaAssessment',assessment);set('sgPaDifferentiation',diff);set('sgPaFollowup',follow);
