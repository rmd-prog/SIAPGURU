(()=>{
/* SIAP GURU — learning chain bridge v2
   TP → RPM → LKPD → Materi
...
const autoLoad=room=>{if(!room)return;const ids=room.classList.contains('sg-lkpd-room')?['sgLLoad']:room.classList.contains('sg-materi-room')?['sgMLoad']:room.classList.contains('sg-rpm-room')?['sgRLoad']:[];ids.forEach(id=>{const b=room.querySelector('#'+id);if(b&&!b.dataset.chainAuto){b.dataset.chainAuto='1';setTimeout(()=>b.click(),120)}})};
const sync=()=>{
