/* SIAP GURU — ATP document preview sync v3
   Event-driven only. Never observes its own DOM mutations. */
(()=>{
  ...
  const sync=room=>{
    ...
    paper.dataset.sgAtpPreviewHtml=html;
    paper.innerHTML=html;
  };
  const schedule=room=>{clearTimeout(room.__sgAtpPreviewTimer);room.__sgAtpPreviewTimer=setTimeout(()=>sync(room),40)};
  const boot=()=>{
    if(window.__sgAtpPreviewSyncV3)return;
    window.__sgAtpPreviewSyncV3=1;
    const room=()=>document.querySelector('.sg-atp-room');
    document.addEventListener('input',e=>{const r=e.target.closest?.('.sg-atp-room');if(r)schedule(r)},true);
    document.addEventListener('change',e=>{const r=e.target.closest?.('.sg-atp-room');if(r)schedule(r)},true);
    document.addEventListener('click',e=>{const r=e.target.closest?.('.sg-atp-room');if(r)schedule(r)},true);
    window.addEventListener('sg:atp-updated',()=>{const r=room();if(r)sync(r)});
  };
