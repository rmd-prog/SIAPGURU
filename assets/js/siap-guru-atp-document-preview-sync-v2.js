(()=>{
  ...
  const boot=()=>{
    document.querySelectorAll('.sg-atp-room').forEach(sync);
    document.addEventListener('input',e=>{const r=e.target.closest?.('.sg-atp-room');if(r)setTimeout(()=>sync(r),0)},true);
    document.addEventListener('change',e=>{const r=e.target.closest?.('.sg-atp-room');if(r)setTimeout(()=>sync(r),0)},true);
