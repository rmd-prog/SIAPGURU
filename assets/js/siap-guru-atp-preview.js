(()=>{
 const make=()=>{
  ...
  const render=()=>{const q=s=>room.querySelector(s),val=s=>q(s)?.value||'';q('[data-doc="name"]').textContent=val('#sgAtpName')||'-';q('[data-doc="semester"]').textContent=val('#sgAtpSemester')==='2'?'Semester 2':'Semester 1';q('[data-doc="subject"]').textContent=val('#sgAtpSubject')||'-';q('[data-doc="phaseclass"]').textContent=`Fase ${val('#sgAtpPhase')||'-'} / Kelas ${val('#sgAtpClass')||'-'}`;q('[data-doc="totaljp"]').textContent=val('#sgAtpTotalJp')?`${val('#sgAtpTotalJp')} JP`:'-';q('[data-doc="group"]').textContent=val('#sgAtpGroup')==='agama'?'Agama & Budi Pekerti':'Mapel Umum';const rows=[...list.querySelectorAll('.sg-atp-item')];let total=0;q('[data-doc="items"]').innerHTML=rows.length?rows.map((it,i)=>{const jp=Number(it.querySelector('.sg-atp-jp')?.value||0);total+=jp;return `<tr><td>${i+1}</td><td>${esc(it.dataset.element)}</td><td>${esc(it.dataset.cp)}</td><td>${esc(it.querySelector('.sg-atp-text')?.textContent||'')}</td><td>${jp}</td></tr>`}).join(''):`<tr><td colspan="5">Belum ada tujuan pembelajaran.</td></tr>`;q('[data-doc="sum"]').textContent=`${total} JP`};
  render();room.addEventListener('input',e=>{if(e.target.matches('input,textarea,select'))render()});room.addEventListener('change',e=>{if(e.target.matches('input,textarea,select'))render()});
  if(window.SiapGuruExport?.attach)window.SiapGuruExport.attach(room);
 };
 const boot=()=>{if(window.__sgAtpPreviewBoot)return;window.__sgAtpPreviewBoot=1;make()};
