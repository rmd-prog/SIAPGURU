...
const groups=[
 {label:'UTAMA',icon:'⌂',name:'Beranda',target:'home'},
...
function build(){if(document.getElementById(ID))return;const side=document.createElement('aside');side.id=ID;side.setAttribute('aria-label','Navigasi SIAP GURU');side.innerHTML=`<div class="sgv2-brand"><div class="sgv2-mark">SG</div><div><div class="sgv2-brand-name">SIAP GURU</div><div class="sgv2-brand-sub">Sistem Informasi Administrasi &amp; Pembelajaran GURU</div></div></div><nav class="sgv2-nav"><div class="sgv2-nav-label">MENU</div>${groups.map((g,i)=>g.items?`<div class="sgv2-group"><button class="sgv2-nav-btn" type="button" data-group="${esc(g.name)}"><span class="sgv2-nav-icon">${g.icon}</span><span>${esc(g.name)}</span><span class="sgv2-chevron">⌄</span></button><div class="sgv2-sub" data-sub="${esc(g.name)}">${g.items.map(x=>`<button type="button" data-subitem="${esc(x)}">${esc(x)}</button>`).join('')}</div></div>`:`<button class="sgv2-nav-btn active" type="button" data-home="true"><span class="sgv2-nav-icon">${g.icon}</span><span>${esc(g.name)}</span></button>`).join('')}</nav><div class="sgv2-side-foot"><strong>SIAP GURU</strong>Tahun Pelajaran 2026/2027</div>`;document.body.appendChild(side);const overlay=document.createElement('div');overlay.className='sgv2-mobile-overlay';overlay.id=OVER;document.body.appendChild(overlay);const mob=document.createElement('button');mob.id=MOBILE;mob.type='button';mob.setAttribute('aria-label','Buka menu');mob.textContent='☰';document.querySelector('.top-actions')?.prepend(mob);
